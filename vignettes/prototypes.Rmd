...
```{r}
ggplot(mtcars) +
  geom_mosaic(aes(x = product(cyl, am)), divider = c("vspine", "hspine"))
```
...
```{r}
# fake data to create an onion

a <- c(1,3,4,5,6,7,7,8,10,10)
...
```{r}
ggplot(mapping = aes(x = mpg)) + 
  geom_density(data = filter(df, cyl == "a"))  + 
  geom_density(data = filter(df, cyl == "b")) + 
  geom_density(data = filter(df, cyl == "c"))
```
```{r}
ggplot(df) +
...
```{r}
ggplot(df) + 
  geom_violin(aes(x = cyl, y = mpg), bw = 1)
```
```{r}
ggplot(tail(mtcars)) + geom_point(aes(x = hp, y =mpg, size = 8)) + theme_classic()
```
```{r}
ggplot(tail(mtcars)) + geom_point(aes(x = hp, y =mpg, size = 8, color = factor(cyl), shape = "9")) + theme_classic()

```
```{r}
ggplot(mtcars) + 
  geom_bar(aes(x = cyl, y = stat(prop), fill = factor(am)), position = "stack")

ggplot(mtcars) + 
  geom_bar(aes(x = cyl, y = stat(prop), fill = factor(am)))
```

```{r}
ggplot(mtcars) + 
  geom_mosaic(aes(x=product(cyl, gear), conds=product(am)), divider = c("vspine", "hbar", "vbar"))
```
