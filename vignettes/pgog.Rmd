...
ggplot(mtcars) + 
  geom_bloc(aes(
    ...
    fill = factor(cyl/2 + 1)), bw = common_bw) + 
  xlab("mpg")
```


Bad example: facet_grid doesn't work well
...
ggplot(mtcars) + 
  geom_bloc(aes(
    ...
    height = c(P(cyl|mpg), P(mpg)), 
    fill = factor(cyl/2 + 1)), bw = common_bw) + 
  xlab("mpg") + facet_grid(gear ~ .)
```


```{r}
# TODO: this is wrong because the grid system is not set up

ggplot(mtcars) + 
  geom_bloc(aes(
    x = c(mpg, gear), 
    fill = factor(cyl), 
    height = c(P(cyl|mpg, gear), P(mpg|gear))
  ))
```
