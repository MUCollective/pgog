...
test_that("parse_aes pmf 10", {
  expect_equal(flatten_aes(m3)$mapping[[2]],quote(gear),ignore.order = TRUE, ignore.case = TRUE)
})
...
test_that("coord_aes 1", {
  expect_named(filter_prob_aes(coord_aes_names,flatten_aes(m3)$mapping),c("x","x"),ignore.order = TRUE, ignore.case = TRUE)
})

test_that("coord_aes 2", {
  expect_equal(filter_prob_aes(coord_aes_names,flatten_aes(m3)$mapping)[[1]],quote(carb),ignore.order = TRUE, ignore.case = TRUE)
})
